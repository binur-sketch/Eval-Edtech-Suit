...
const Navbar = () => {
  ...
  const solutions = [
    ...
    { title: 'Question Bank', path: '/qtb' },
    { title: 'On-Screen Marking', path: '/osm' },
  ];

  const resourceLinks = [
    { title: 'Our Clients', path: '/clients' },
    { title: 'Insights & Blog', path: '/resources' },
    { title: 'About Vir Softech', path: '/about' },
  ];
  ...
  return (
    <header style={{ position: 'fixed', width: '100%', zIndex: 1000, top: 0 }}>
      ...
      <div style={{ 
        ...
        <div className="container" style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '2rem' }}>
      ...
      {/* Main Navbar - Based on Reference */}
      <nav style={{ 
        background: 'white', 
        ...
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: '100%', width: '100%' }}>
          ...
          {/* Nav Items Container - Pushed to the Right */}
          <div className="md-flex" style={{ alignItems: 'center', height: '100%' }}>
            ...
            <div className="dropdown-container" style={{ position: 'relative', height: '100%', display: 'flex', alignItems: 'center' }}>
              ...
              <div className="dropdown" style={{ top: '100%', left: '0' }}>
                {solutions.map((s, i) => (
                  <Link key={i} to={s.path} className="dropdown-item">{s.title}</Link>
                ))}
              </div>
            </div>
            ...
            <div className="dropdown-container" style={{ position: 'relative', height: '100%', display: 'flex', alignItems: 'center' }}>
              ...
              <div className="dropdown" style={{ top: '100%', left: '0' }}>
                {resourceLinks.map((r, i) => (
                  <Link key={i} to={r.path} className="dropdown-item">{r.title}</Link>
                ))}
              </div>
            </div>
            ...
            <Link to="/book-demo" className="btn btn-primary" style={{ padding: '0.8rem 2.5rem', marginLeft: '1.5rem', fontSize: '0.8125rem', borderRadius: '4px', fontWeight: '800' }}>
              Book a Demo
            </Link>
          </div>
        ...
        {isOpen && (
          <div className="mobile-menu reveal" style={{ 
            ...
            <div>
              <div style={{ fontWeight: '900', marginBottom: '1.5rem', color: 'var(--primary)', fontSize: '0.8125rem', textTransform: 'uppercase', letterSpacing: '2px' }}>Solutions</div>
              <div style={{ display: 'grid', gap: '1.25rem', paddingLeft: '0.5rem' }}>
                {solutions.map((s, i) => (
                  <Link key={i} to={s.path} onClick={() => setIsOpen(false)} style={{ fontSize: '1.125rem', fontWeight: '700', color: 'var(--secondary)' }}>{s.title}</Link>
                ))}
              </div>
            </div>

            <div>
              <div style={{ fontWeight: '900', marginBottom: '1.5rem', color: 'var(--primary)', fontSize: '0.8125rem', textTransform: 'uppercase', letterSpacing: '2px' }}>Resources</div>
              <div style={{ display: 'grid', gap: '1.25rem', paddingLeft: '0.5rem' }}>
                {resourceLinks.map((r, i) => (
                  <Link key={i} to={r.path} onClick={() => setIsOpen(false)} style={{ fontSize: '1.125rem', fontWeight: '700', color: 'var(--secondary)' }}>{r.title}</Link>
                ))}
              </div>
            </div>
